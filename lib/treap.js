...
//    * Supports fast concatenation and splitting
//    * Has augmentations for size and edge levels
//    * Node references are not invalidated during updates

module.exports = createTreap

function TreapNode(value, count, priority, parent, left, right, next, prev) {
  this.value = value
  this.count = count
...
proto.insert = function(value) {
  if(!this.right) {
    var nn = this.right = new TreapNode(value, 1, Math.random(), this, null, null, this.next, this)
    if(this.next) {
      this.next.prev = nn
    }
    this.next = nn
  ...
  var v = this.next
  var nn = v.left = new TreapNode(value, 1, Math.random(), v, null, null, v, this)
  v.prev = nn
  this.next = nn
  nn.bubbleUp()
  return nn
...
function createTreap(value) {
  return new TreapNode(value, 1, Math.random(), null, null, null, null, null)
}